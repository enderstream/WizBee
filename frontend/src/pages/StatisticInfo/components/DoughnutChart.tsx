...
const DoughnutChart: React.FC = () => {
  ...
  const activityData = {
    순공: { value: 7, color: '#3b82f6', unit: '시간' },
    폰: { value: 25, color: '#50C878', unit: '분' },
    졸음: { value: 35, color: '#a855f7', unit: '분' },
    '자리 비움': { value: 1, color: '#ef4444', unit: '시간' },
  }
  ...
  const chartData = {
    ...
    datasets: [
      {
        data: Object.values(activityData).map((item) =>
          item.unit === '시간' ? item.value : item.value / 60,
        ),
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const targetWidths = Object.entries(activityData).reduce(
        (acc, [key, data]) => {
          acc[key] =
            ((data.unit === '시간' ? data.value : data.value / 60) /
              totalTime) *
            100
          return acc
        },
