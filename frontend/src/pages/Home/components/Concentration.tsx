...
const Concentration: React.FC<ConcentrationProps> = ({
  ...
  // 집중도 계산 및 포맷팅
  const concentrationPercent = concentrationData ? Math.round(concentrationData.data * 100) : 0;
  
  // 집중도에 따른 이모지와 멘트
  const getFeedback = (percent: number) => {
    if (percent >= 80) return { emoji: '🎯', message: '최고의 집중력!!!' };
    if (percent >= 60) return { emoji: '👍', message: '좋은 집중력!!' };
    if (percent >= 40) return { emoji: '🔍', message: '조금만 더 화이팅!' };
    return { emoji: '😴', message: '휴식이 필요해요...' };
  };
    ...
    <div className="mb-6 bg-gradient-to-br from-purple-100 to-purple-200 rounded-2xl p-3 shadow-sm relative overflow-hidden h-[88px]">
      <div className="flex items-center h-full">
        ...
        <div className="pl-3 flex-1 flex flex-col justify-center">
          <h1 className="text-center font-bold text-purple-800 text-base">오늘의 집중 효율</h1>
          
          <div className="flex justify-center items-center mt-1">
            {/* 수치 영역 */}
            <div className="flex items-baseline pr-2">
              <span className="text-2xl font-bold text-purple-900">{concentrationPercent}</span>
            ...
            <div className="flex items-center pl-2">
              ...
              <span className="text-sm text-purple-700 font-medium">{feedback.message}</span>
            </div>
          </div>
        </div>
      </div>
