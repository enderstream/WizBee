...
while True:
    ...
    if results.multi_face_landmarks:
        ...
        for iris_indices, label in zip([LEFT_IRIS_IDX, RIGHT_IRIS_IDX], ["Left", "Right"]):
            points = [(int(face_landmarks.landmark[i].x * w), int(face_landmarks.landmark[i].y * h)) for i in iris_indices]
            cx = int(np.mean([p[0] for p in points]))
            cy = int(np.mean([p[1] for p in points]))
            if 0 <= cy < h and 0 <= cx < w:
                value = frame[cy, cx][0]  # 파란 채널
                cv2.circle(frame, (cx, cy), 3, (0, 255, 255), -1)
                cv2.putText(frame, f"{label} Iris: {value}", (cx + 5, cy), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)
                if label == "Left":
                    left_closed = value > 60
                else:
                    right_closed = value > 60
                eye_detected = True
    ...
    if eye_detected:
        ...
        cv2.putText(frame, f"Eye Status: {eye_status}", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
    else:
